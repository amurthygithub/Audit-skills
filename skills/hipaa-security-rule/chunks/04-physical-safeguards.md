...
All four specs are Addressable, and the disposition turns on whether the organization itself **operates a facility housing ePHI systems**. A BA that does (an office server room, a colocation cage) dispositions these specs `implement` or `alternative_measure`. A fully-remote organization with **no organization-controlled facility** dispositions them `not_reasonable_documented`: the why-not record AND the alternative-considered answer (§164.306(d)(3)(ii)(B)) together document the inheritance of the cloud provider's data-center controls (reviewed via the provider's SOC 2 report) — see `use-cases/uc-01-ba-risk-analysis.md` for the worked no-facility example and `industries/saas-technology.md` for the shared-responsibility framing. **Evidence:** badge/key inventories, visitor logs, facility security plan, security-relevant maintenance tickets.
