"""Telemetry tests for audit-workpapers.
...
def _full_event(**overrides):
    base = {
        ...
        "latency_ms": 0,
        "cache_hit": False,
        "classification": "MUS_SAMPLE_SIZE_75",
        "oracle_result": "n/a",
...
def test_skill_name_pattern():
    inv = _full_event(skill="audit-workpapers")
    jsonschema.validate(inv, SCHEMA)
    bad = _full_event(skill="AUDIT WORKPAPERS")
    with pytest.raises(jsonschema.ValidationError):
        jsonschema.validate(bad, SCHEMA)


def test_use_case_id_pattern():
    for uc in ["UC-01", "UC-02", "UC-03"]:
        inv = _full_event(use_case_id=uc)
        jsonschema.validate(inv, SCHEMA)
...
def test_classification_audit_specific():
    """Classification values must match audit-workpapers domain."""
    for cls_val in ["MUS_SAMPLE_SIZE_75", "SIGNIFICANT_DEFICIENCY", "MATERIAL_WEAKNESS", "TD_20.8"]:
        inv = _full_event(classification=cls_val)
        jsonschema.validate(inv, SCHEMA)
...
def test_instrument_emits_event():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        @instrumented(skill="audit-workpapers", skill_version="0.2.0", default_use_case_id="UC-01", default_industry="financial-services")
        def fake_run(payload, **kwargs):
            return {"classification": "MUS_SAMPLE_SIZE_75", "input_tokens": 100, "output_tokens": 50}

        out = fake_run({"population_book_value": 12500000})
        ...
        assert event["skill"] == "audit-workpapers"
        assert event["classification"] == "MUS_SAMPLE_SIZE_75"
        assert event["use_case_id"] == "UC-01"


def test_skill_version_pattern():
    inv = _full_event(skill_version="0.2.0")
    jsonschema.validate(inv, SCHEMA)
    bad = _full_event(skill_version="v0.2")
    with pytest.raises(jsonschema.ValidationError):
        jsonschema.validate(bad, SCHEMA)
