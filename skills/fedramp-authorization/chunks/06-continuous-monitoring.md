...
- **SLA currency (G4.5 §5.11):** 30/90/180 is the governing **Rev 5** ConMon standard as of 2026-06 (the day counts come from the FedRAMP Continuous Monitoring Strategy Guide / vulnerability-scanning requirements; the ConMon *overview* page states the monthly cadence, not the day counts). FedRAMP's **Vulnerability Detection and Response (VDR) Standard / RFC-0012** — exploitability- and reachability-based (e.g. ~3-day internet-reachable) — is in flight and already effective for **FedRAMP 20x Low**, but is **not yet** in force for Rev 5 (open beta tentatively ~Oct 2026). Re-verify before relying on these windows.
