...
| 5 | The LI-SaaS 156 splits **66** 3PAO-tested + **90** CSP-attested | FEDRAMP-REV5-BASELINES (Tailored LI-SaaS doc); fact sheet §0 `li_saas_3pao_tested` / `li_saas_attested` | the 66/90 split is documented in the Tailored LI-SaaS baseline doc (cited to the doc, NOT a flat OSCAL count) | VERIFIED |
...
| 27 | UC-02 derivation: LI-SaaS eligible iff `overall == Low AND saas_delivery`; eligible → 156 = 66 tested + 90 attested; Moderate+SaaS → NOT eligible (full Moderate, 323) | recomputation from `data/seeds/uc-02-*.json` | oracle re-derives eligibility from the seed and the fixed 66/90 split; the Moderate+SaaS perturbation is refused as ineligible | VERIFIED |
