...
| [UC-02](uc-02-li-saas-readiness.md) | Cloud vendor LI-SaaS readiness — Beacon Forms (all-Low CIA, SaaS-delivered) | saas-technology | cloud-vendor founder | LI-SaaS **eligible** (Low + SaaS); baseline **156 = 66 3PAO-tested + 90 CSP-attested** (`LI_SAAS_ELIGIBLE`); Moderate+SaaS is the trap (not LI-SaaS) |
...
5. The framework constants (the baseline counts 156/323/410 and the 66/90 LI-SaaS split) are fixed from the PMO OSCAL profiles — the **derivation** is the seed-driven selection/computation (high-water mark, baseline lookup, due-date = date + SLA, finding selection), not the constants.
...
- **DoD Impact Levels / DISA SRG, StateRAMP, CMMC** — distinct regimes, named as adjacent, not covered.
