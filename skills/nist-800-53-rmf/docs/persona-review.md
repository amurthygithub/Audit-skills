...
| Partner, SaaS | UC-03 headline (231 mapped / 71% overlap / 94 gaps) is not derivable from the shipped crosswalk (33 rows, ~27-39 unique controls ≈ 8%); stub hardcodes the headline; oracle asserts the hardcoded constants (circular); gap list has 93 records vs stated 94; 12 controls appear as both mapped and gap. | Verified: gap list = 93 records (counted); chunk 09 admits ~8% | **FIXED 2026-06-10 (SOX-637, M2)** — headline restated to what the data supports (curated sample: 33 mappings / 39 unique IDs; 93-record register = 81 gap + 12 strengthen dispositions); stub now COMPUTES the summary from the seeds; oracle recomputes independently (non-circular) and asserts footing invariants; no overlap %% is emitted (not derivable from a sample); the complete TSC→800-53 mapping remains an engagement input / future fact-sheet job |
| CISO | HIPAA crosswalk seed has 12 mappings vs ~50+ Subpart C specs; 164.308(a)(5)(ii)(C) (log-in monitoring) wrongly mapped to AT-2 as "exact"; all of 164.310 physical safeguards absent; no break-glass row. | Verified row count + the AT-2 row on disk | **FIXED (data) 2026-06-10 — SOX-638**: seed rebuilt from the NIST CPRT OLIR set (68 elements / 279 rows / 108 controls; AT-2 row corrected to AT-3+AU-6 per CPRT — note the persona-proposed AC-7/AU-2/AU-6 was ALSO wrong; strength ratings dropped, the OLIR source carries none). Healthcare UC remains open under this ticket. |
| State Dir | "Public sector" industry view covers only federal contexts; SKILL.md:40 promises "state-RMF" support that no content backs; authorization model assumes federal AO/CFACTS/FedRAMP machinery with no state/local or non-federal translation. | Verified on disk | **FIXED 2026-06-11 (SOX-639 slice, PR pending)** — new `chunks/10-non-federal-adoption.md` backs the promise: the non-federal authorization model (who risk-accepts absent a federal AO), state RMF adoption, resource-constrained risk-based scoping, and the non-federal report shape + a GAGAS finding pointer; SKILL.md activation routed to it; public-sector.md cross-references it. Data-specific overlays + consumer affordances split to own tickets (see below). |
...
| 3PAO | Baseline counts presented as NIST but approximating FedRAMP; SP 800-53B (the actual baseline source) uncited. | **FIXED** — limits doc distinguishes SP 800-53B vs FedRAMP workbook; SP-800-53B added to manifest + registry 2026-06-10 (SOX-637, M2) |
...
| State Dir | IRS Pub 1075 in manifest but unused; MARS-E/SSA overlays absent for the benefits-eligibility scenario; assessment procedure offers no risk-based scoping for small shops. | **PARTIAL FIX 2026-06-11 (SOX-639 slice)** — resource-based scoping for small shops shipped (chunk 10 §3: family prioritization, sampling, prior-evidence reuse, multi-year rotation). The data-specific federal overlays (IRS Pub 1075 / MARS-E / SSA benefits-eligibility) split to a new ticket. |
...
| SaaS | Procedures end at "document in SSP §2/§8/§9/§10" with no SSP template shipped or linked; no sponsorship/3PAO/before-you-start path. | **TICKETED (split from SOX-639)** — consumer affordances (SSP/CRM template links at each "document in SSP §N" step, sponsorship/3PAO/before-you-start, board-legible output) → own ticket; out of the SOX-639 correctness slice. |
...
- "overlap_pct [68,75] industry-typical" uncited; "~64 criteria with points of focus" — **FIXED 2026-06-10 (SOX-637, M2)**: both removed; no overlap %% emitted.
- financial-services.md "no native cloud inheritance" contradicts UC-03's AWS scenario — **FIXED 2026-06-10 (SOX-637, M2)**: qualified by deployment model.
