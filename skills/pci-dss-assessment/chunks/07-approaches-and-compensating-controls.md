...
topic: "Defined vs customized approach (Appendix D/E; TRA); requirements with no customized option; compensating controls (Appendix B/C) and the four worksheet elements; how customized and compensating differ and when to use each"
...
The **compensating-controls worksheet (Appendix C)** requires **exactly four elements**:

1. **Constraints** — the legitimate business/technical constraint preventing the defined requirement.
2. **Objective** — the objective of the original requirement.
3. **Risk** — the additional risk posed by not meeting the defined requirement.
4. **Controls in place** — the compensating controls and how they meet the requirement's intent/rigor.

A worksheet is **complete iff all four elements are present**. Compensating controls must go **above and beyond** the defined requirement and address the additional risk the constraint introduces.
...
| Required analysis | **Targeted Risk Analysis (TRA)** | **Four-element worksheet** |
...
3. If the entity **cannot** meet the defined requirement due to a **legitimate constraint** → **compensating control**: complete the **four-element worksheet** and show the controls exceed the defined requirement.
