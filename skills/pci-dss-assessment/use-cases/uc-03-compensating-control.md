---
uc_id: UC-03
title: "Solo QSA-support consultant validates a compensating control for a legacy POS that cannot meet a Req-8 auth control: all six Appendix-C worksheet rows present, worksheet complete (Meridian QSA-Support)"
...
inputs:
  ...
  proposed_compensating_control: "the six Appendix-C worksheet rows (constraints_documented, compensating_controls_defined, objective_of_original_requirement_stated, identified_risk_documented, validation_of_compensating_controls, maintenance_documented) all true; controls = network isolation of legacy POS + MFA at the jump host + enhanced logging/alerting"
...
procedure:
  ...
  - "chunks/07-approaches-and-compensating-controls.md — Check the Appendix-C worksheet for all six rows: Constraints, Definition of Compensating Controls, Objective, Identified Risk, Validation of Compensating Controls, and Maintenance; the worksheet is complete iff all six are present."
...
oracle:
  ...
  - "worksheet_elements_present recomputed as the count of present Appendix-C rows == 6; missing_elements == []"
  - "worksheet_complete == true iff no row is missing; Appendix C has exactly six rows"
  - "classification == COMP_CONTROL_COMPLETE, derived from completeness"
  - "removing any one row flips worksheet_complete to false and the classification to COMP_CONTROL_INCOMPLETE"
data_refs:
...
# UC-03 — Compensating control for a legacy POS: the six-row Appendix-C worksheet (Meridian QSA-Support)
...
Because Meridian's situation is a **legitimate constraint against an existing defined requirement**, the correct mechanism is a **compensating control** — `control_type = compensating_control`. The Appendix-C documentation requirement has **six** "Information Required" rows, not four — a common undercount. The classification is constraint-driven: remove the legitimate constraint and the same facts would instead be a **customized approach** (the adversarial test `test_uc03_no_constraint_is_customized_not_compensating` exercises that flip). This is the same distinction UC-02 makes from the customized-approach side.

## §3 The Appendix-C worksheet — all six rows present

A compensating control is only valid if it is documented on the **Compensating Controls Worksheet (Appendix C)** with all six "Information Required" rows present. The proposed control carries every one:

| # | Appendix-C row | Seed field | Present |
|---|---|---|---|
| 1 | Constraints (why the defined requirement cannot be met) | `constraints_documented` | yes |
| 2 | Definition of Compensating Controls (the controls themselves and how they meet intent and rigor) | `compensating_controls_defined` | yes |
| 3 | Objective of the original requirement | `objective_of_original_requirement_stated` | yes |
| 4 | Identified Risk (introduced by the constraint) | `identified_risk_documented` | yes |
| 5 | Validation of Compensating Controls (how the controls were validated/tested) | `validation_of_compensating_controls` | yes |
| 6 | Maintenance (how the controls are maintained over time) | `maintenance_documented` | yes |
...
**All six rows present → worksheet complete.** `worksheet_elements_present = 6`, `missing_elements = []`, `worksheet_complete = true`, `classification = COMP_CONTROL_COMPLETE`.
...
Metamorphic: evaluating the six rows does not depend on dict key order (`test_uc03_element_order_invariance`). Adversarial: setting any one row to false (e.g. `identified_risk_documented`) flips `worksheet_complete` to false and the classification to `COMP_CONTROL_INCOMPLETE` (`test_uc03_incomplete_worksheet_flagged`); removing the legitimate constraint reclassifies the mechanism as a customized approach (`test_uc03_no_constraint_is_customized_not_compensating`).

## §5 Anti-hallucination

- **Meridian QSA-Support is fictional**; the seeds are the tested fixture and this UC's source of truth.
- **Compensating control ≠ customized approach.** A compensating control (Appendix B/C) is constraint-driven against an existing requirement and meets its intent and rigor via the six-row worksheet; a customized approach (Appendix D/E) is an objective-met-differently design choice requiring a TRA and is not constraint-driven [PCI-SSC-Document-Library §AppendixC].
- **The worksheet is complete only with all six rows** — Constraints, Definition of Compensating Controls, Objective, Identified Risk, Validation of Compensating Controls, Maintenance; a missing row is an incomplete worksheet, never a silent pass.
