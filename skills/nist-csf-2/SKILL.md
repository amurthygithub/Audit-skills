...
risk: informational
source: "NIST Cybersecurity Framework 2.0 (Feb 26, 2024) — https://www.nist.gov/cyberframework. Companion: NIST SP 1299 (Resource and Overview Guide), SP 1300 (Small Business Quick-Start Guide), SP 1301 (Organizational Profiles QSG), SP 1302 (Tiers QSG), CSF 2.0 Informative References spreadsheet, FIPS 199, NIST SP 800-53 Rev 5 / 5.1.1, NIST SP 800-37 Rev 2 RMF, FedRAMP Rev 5, SOC 2 TSC 2017, ISO 27001:2022, HIPAA Security Rule, PCI DSS v4.0.1, COBIT 2019, NY DFS Part 500, CISA Cross-Sector CPGs."
...
The NIST Cybersecurity Framework 2.0 [NIST-CSF-2.0 §2] is a voluntary, outcome-based framework published by NIST on February 26, 2024 (mandatory for federal agencies per EO 13800). It organizes cybersecurity risk into **6 Functions** [NIST-CSF-2.0 §2.1], **22 Categories**, and **106 Subcategories**, characterizes the rigor of risk governance/management with **4 Tiers** applied to Organizational Profiles [NIST-CSF-2.0 §3.2] (Tiers are NOT maturity levels), and describes posture with **4 Profile types** (Current, Target, Organizational, Community). It is the most executive-legible framework in the NIST family and designed for **both** IT practitioners and executive decision-makers. Companion documents include NIST SP 1299 (Resource and Overview Guide) [NIST-SP-1299] and NIST SP 1300 (Small Business Quick-Start Guide) [NIST-SP-1300]. Regulatory alignment spans OMB A-130 [OMB-A-130], NY DFS Part 500 [NY-DFS-Part-500], and FedRAMP Rev 5 baselines [FedRAMP-Rev5]. Its informative references spreadsheet [CSF-2.0-Informative-References] provides crosswalks to NIST SP 800-53 Rev 5.1.1 [NIST-SP-800-53-Rev5.1.1], NIST SP 800-37 Rev 2 RMF [NIST-SP-800-37-Rev2], NIST SP 800-30 Rev 1 risk assessments [NIST-SP-800-30-Rev1], NIST SP 800-61 Rev 2 incident handling [NIST-SP-800-61-Rev2], and sector guidance such as the CISA Cross-Sector CPGs [CISA-CPG].
...
| Rigor | 4 Tiers: 1 (Partial), 2 (Risk Informed), 3 (Repeatable), 4 (Adaptive) | The "how rigorously governed" — applied to Organizational Profiles; NOT maturity levels. |
...
**Relationship to other NIST frameworks:** CSF 2.0 outcomes (Subcategories) map to 800-53 controls via the NIST Informative References spreadsheet — the authoritative crosswalk source (see `chunks/08-informative-references-crosswalk.md`; this skill deliberately ships no derivative JSON snapshots). The word "Tier" collides across NIST constructs: CSF **Tiers 1-4** characterize risk-governance rigor (applied to Organizational Profiles, not maturity levels); SP 800-39/800-37 **Tiers 1-3** are organization/mission/system LEVELS of the risk-management hierarchy; FIPS 199 defines impact **levels** (Low/Moderate/High — never "Tiers"). See `chunks/02-tiers-and-profiles.md` for the reconciliation.
...
Categories are the second-level grouping. Each Category has 1-10 Subcategories (e.g., GV.PO has 2; GV.SC has 10). Representative lists are in `chunks/01-functions-categories.md`; the canonical 106-row inventory is the NIST CSF 2.0 PDF Appendix A / CSRC reference export (this skill ships no derivative JSON). Examples: `GV.OC` (Organizational Context), `GV.SC` (Cybersecurity Supply Chain Risk Management), `ID.AM` (Asset Management), `PR.AA` (Identity Management, Authentication, and Access Control — renamed in 2.0), `DE.CM` (Continuous Monitoring), `RS.MI` (Incident Mitigation), `RC.RP` (Incident Recovery Plan Execution).

### 3.3 Tiers 1-4 (rigor of risk governance — NOT maturity levels)
...
NIST applies Tiers to **Organizational Profiles** [NIST-CSF-2.0 §3.2] and is on record that Tiers are not maturity levels. Scoring a separate Tier per Function (the 6-function radar in `chunks/06-enterprise-reporting.md`) is a common **practitioner extension** — label it as the org's own convention in any deliverable.
...
- **Tier selection is not normative** in CSF 2.0 [NIST-CSF-2.0 §3.2] — see `chunks/02-tiers-and-profiles.md` for the heuristic.
- **GOVERN-first assessment** (practitioner heuristic supported by GOVERN's structural placement) — assess GOVERN before the other 5 Functions because it sets the context. See `chunks/03-current-profile.md`.
- **There is no "800-53 Tier"** — CSF Tiers 1-4 = risk-governance rigor; SP 800-39/800-37 Tiers 1-3 = organization/mission/system levels; FIPS 199 = impact LEVELS (L/M/H). Three constructs, one colliding word — see `chunks/02-tiers-and-profiles.md`.
...
- **CSF → 800-171 Rev 2 mapping for CMMC L2** (DoD supplier) — `use-cases/uc-03-csf-to-800-171-cmmc-l2.md`.
...
External: NY DFS Part 500, FFIEC IT Examination Handbook (the CAT was sunset Aug 31, 2025), CISA Cross-Sector CPGs, FIPS 199, FedRAMP Rev 5, CMMC 2.0, ISO 27701, HITRUST CSF (related-but-distinct).
...
|----|-------|----------|------------|
| UC-01 | First-profile pattern: 50-FTE Series-A SaaS (DataRelay), Tier 1→2 in 90 days, 5-item roadmap | saas-technology | Current Profile, gap prioritization, 90-day roadmap |
| UC-02 | Board cyber report: $24B regional bank (Pinecrest), 6-function radar, 12-month $2.0M capital plan | financial-services | Radar, GOVERN narrative (all 6 GV Categories), capital plan |
| UC-03 | CSF → 800-171 Rev 2 mapping for CMMC L2: 240-FTE aerospace DoD supplier (Apex Manufacturing), 14 lagging Subcategories | manufacturing | Crosswalk to 800-171 Rev 2 controls, CMMC L2 readiness scorecard |
...
- **There is no "800-53 Tier."** CSF Tiers 1-4 characterize risk-governance rigor, applied to Organizational Profiles, and are not maturity levels. SP 800-39/800-37 Tiers 1-3 are organization/mission/system LEVELS. FIPS 199 defines impact LEVELS (Low/Moderate/High), never "Tiers". Per-Function or fractional Tier scores are practitioner conventions — label them as such; do not present them as NIST methodology.
...
- **No quantitative risk score and no official implementation-status scale** — CSF 2.0 prescribes neither. The Not/Partially/Largely/Fully status scale and any Tier rollup/averaging in this skill are documented house conventions (SP 1301's Profile example is explicitly notional/free-format). Do not invent a numeric score or attribute scales to NIST.
...
| NIST-SP-1300 | NIST Cybersecurity Framework 2.0: Small Business Quick-Start Guide | NIST | SP 1300 (Feb 2024) | 2026-06-10 | https://csrc.nist.gov/pubs/sp/1300/final |
| NIST-SP-1299 | NIST Cybersecurity Framework 2.0: Resource and Overview Guide | NIST | SP 1299 (Feb 2024) | 2026-06-10 | https://csrc.nist.gov/pubs/sp/1299/final |
...
**Use cases** (load matching file from `use-cases/`): UC-01 (first profile, Series-A SaaS), UC-02 (board report, regional bank), UC-03 (CSF → 800-171 Rev 2 / CMMC L2, DoD supplier).
...
For board/executive questions → `chunks/05-govern-function.md` (GOVERN is the executive language). For federal/DoD/FedRAMP → hand off to `nist-800-53-rmf`; the authoritative CSF crosswalk source is the NIST Informative References spreadsheet (see `chunks/08` — this skill ships no derivative JSON).
