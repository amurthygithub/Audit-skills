...
An enterprise SaaS might receive 50-200 unique security questionnaires per year, each with 100-300 questions. CAIQ v4 (Cloud Security Alliance Consensus Assessments Initiative Questionnaire) and SIG Lite (Standardized Information Gathering) are attempts to standardize, but customers often send their own variant. A CSF 2.0 Current Profile (scoring all 106 Subcategories) provides a single source of truth: when a customer asks about access control, you point to `PR.AA-01` through `PR.AA-06`; when they ask about incident response, you point to `RS.MA-01`, `RS.AN-03`, `RS.CO-02`. A well-maintained CSF Profile can pre-answer a large share of CAIQ v4 / SIG / VSAQ items (roughly 60% is this skill's unsourced practitioner estimate — [INTERPRETIVE]); the remainder are product-specific technical questions that CSF 2.0 does not address.
...
This is a documentation-and-configuration exercise, not a tool-acquisition exercise. The 50-FTE archetype in UC-01 (`use-cases/uc-01-first-organizational-profile.md`) is fictional; replace with a real engagement when adapting.
...
- Customer questionnaire fatigue is real: an enterprise SaaS might receive 50-200 unique security questionnaires per year, each with 100-300 questions. CAIQ v4 (Cloud Security Alliance) and SIG Lite (Shared Assessments) are attempts to standardize. CSF 2.0 is a way to maintain a single source of truth — the Current Profile with per-Subcategory scores and evidence references — that can pre-answer a large share of questionnaire items (roughly 60% — unsourced practitioner estimate, [INTERPRETIVE]); the remainder are product-specific technical questions that CSF does not address.
